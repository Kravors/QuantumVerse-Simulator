...
> The 4D UI has been successfully implemented using Dear ImGui + GLFW, replacing the original Qt-based design. See `MIGRATION_SUMMARY.md` for details.
...
The 4D UI sits as a module under `src/ui_imgui/`, using the existing `spacetime`, `physics`, and `rendering` libraries. The `CurvatureRenderer` accepts a slicing hyperplane parameter and outputs 4D‑ready vertex data. The `DiscoveryPanel` shows 4D‑specific alerts (e.g., a closed time‑like curve appearing as a loop in the causal graph).
...
This 4D UI transforms QuantumVerse from a simulator with a 3D viewport into a **true laboratory for four‑dimensional spacetime cognition**. Every dimension is exposed, every causal relation is visible, and every exotic configuration can be directly shaped. The UI plan is fully compatible with the existing C++/OpenGL architecture and has been implemented incrementally.
...
|---------|--------|----------------|
| 4D Camera | ✅ Complete | `Camera4D` class with 6-plane rotation, translation, zoom |
| Slice Views | ✅ Complete | `SliceViewPanel` - 4 simultaneous 3D hypersurface views |
| Planck Microscope | ✅ Complete | `PlanckMicroscopePanel` - Logarithmic scale control (10^-35 to 10^0 m) |
| Discovery Panel | ✅ Complete | `DiscoveryPanel` - Curvature scalars, anomaly table |
| Causal Graph | ✅ Complete | `CausalGraphPanel` - Event nodes with causal future/past |
| Theory Manager | ✅ Complete | `TheoryManagerPanel` - GR, CDT, LQG, GFT, Causal Sets, Dilaton |
| Context Menu | ✅ Complete | `ContextMenu.h` - Physics tool callbacks |
| Anomaly Alerts | ✅ Complete | `FloatingPanels.h` - Real-time notifications |
| Real-time Curvature Overlay | ✅ Complete | Toggle via 'C' key |
