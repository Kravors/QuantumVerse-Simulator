...
int main() {
    ...
    // --- SymbolicMath availability check --------------------------------------
    {
        ...
        if (available) {
            // --- Einstein equations ------------------------------------------------
            {
                ...
                QV_CHECK(result.is_valid);
                QV_CHECK(!result.expression.empty());
            ...
            {
                QV_CHECK(sym.validateFieldEquation("R_uv - 1/2 R g_uv = 8 pi T_uv"));
    ...
    // --- SymbolicRegressionEngine -----------------------------------------------
    {
        ...
        QV_CHECK(result.complexity >= 0);
        QV_CHECK(result.fitness_score >= 0.0 || result.fitness_score == 0.0);
    }
