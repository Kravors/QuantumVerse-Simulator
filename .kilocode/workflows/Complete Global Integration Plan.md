...
- [ ] Set up CMake with GLFW, ImGui (docking), Glad, stb_image.
- [ ] Create `main_glfw_imgui.cpp` – basic window + ImGui init.
- [ ] Port UI4D manager to ImGui (no Qt signals/slots → observer pattern).
- [ ] Implement `Texture` class (load from disk, generate mipmaps, bind).
- [ ] Convert `CelestialBodyRenderer` to use `sampler2DArray`.
- [ ] Write planet shader (diffuse, specular, normal mapping).
- [ ] Download NASA textures: Earth, Jupiter, Sun (8K where available).

### Phase 1 Core – Subtasks
- [ ] `SpacetimePoint` struct (t, x, y, z; imaginary flag).
- [ ] `MetricTensor` virtual class; implement `MinkowskiMetric`, `SchwarzschildMetric`.
- [ ] `GeodesicIntegrator` (RK4, Dormand‑Prince) with step‑size control.
- [ ] `CurvatureCalculator` – compute Riemann via symbolic diff or finite difference.
- [ ] `Camera4D` – position, look‑at, up1 (timelike), up2 (spacelike). Projection matrix 4×4 → 3×3.
- [ ] `SliceView` widget – choose slice type (t‑const, tau‑const, null). Render as FBO texture → ImGui::Image.
- [ ] `LightCone` – for a given event, sample null geodesics and generate mesh.

### Phase 2 Advanced – Subtasks
- [ ] `BlackHole` class – mass, spin, charge; event horizon radius; ergosphere.
- [ ] `TopologySwitcher` – change open‑set logic in rendering (Zeeman: refine along null/timelike directions).
- [ ] `ComplexCoordinates` – split real/imag, Wick rotation method.
- [ ] `TimeGeography` – `CapabilityPrism` compute reachable events via geodesic shooting.
- [ ] `PlanckMicroscope` – zoom slider (log10 meters); LOD thresholds (1e-15, 1e-30, 1e-35). Switch renderer.

### Phase 3 AI – Subtasks
- [ ] `DifferentiableGeodesicIntegrator` – wrap numeric integrator, record Jacobians.
- [ ] Python training scripts using PyTorch (for neural ODE, GNN, flow).
- [ ] ONNX export and C++ inference (via ONNX Runtime or LibTorch).
- [ ] `RLAgent` – stable‑baselines3 integration (C++ → Python bridge or pure C++).
- [ ] `BayesianEvidence` – nested sampling (dynesty) or MCMC (PyMC) bridge.
- [ ] `PaperGenerator` – collect discovery data, fill LaTeX template, call `pdflatex`.

### Phase 4 GUI/VR – Subtasks
- [ ] Extend ImGui panels: object browser (tree), property editor (sliders), discovery log (table), timeline (play/pause/speed).
- [ ] Implement docking layout (central view + right/left/bottom docks).
- [ ] WebRTC server (Node.js or C++ with libdatachannel).
- [ ] VR integration – OpenXR (SteamVR/Oculus) with Camera4D binding.
...
- [ ] Profiling with Tracy or Intel VTune.
- [ ] Replace RK4 geodesic with neural ODE (fallback for >1% error).
- [ ] GPU compute shaders for curvature grid evaluation (GLSL compute).
- [ ] Implement BVH for metric queries (spatial index in 4D).
- [ ] Multi‑threading: worker pool for N‑body force calculations.
