...
The new GUI will **not** replace the existing UI4D/Camera4D/SliceView modules; it will **wrap** them in a higher‑level application shell using **Qt Quick** (QML) for layout and **QOpenGLWidget** (or `QQuickFramebufferObject`) for the 3D viewport. This preserves all battle‑tested 4D rendering logic, geodesic calculations, and quantum geometry visualisation.

```mermaid
graph TD
    A[QML GUI Layer<br/>(Qt Quick)] --> B[OpenGL Viewport<br/>(QQuickFramebufferObject)]
    A --> C[Sidebar Panels]
    A --> D[Timeline Controls]
    B --> E[CurvatureRenderer<br/>QuantumGeometryRenderer]
    E --> F[Spacetime/Physics/Quantum Modules]
    B --> G[Camera4D<br/>(wrapped in C++)]
    C --> H[Object Manager<br/>(scene graph)]
...
- `SceneGraphManager`: Holds a structured list of all sim objects (stars, black holes, particles, quantum geometries). Each object exposes properties (mass, spin, position, worldline, metric parameters) via Qt properties for QML binding.
- `Camera4DAdapter`: Exposes camera controls to QML through signals/slots; maps mouse/keyboard events to 6‑rotation‑plane manipulation.
...
- `DiscoveryUIMediator`: Bridges the DiscoveryEngine and AI modules to QML panels.
...
- User interaction (QML) → C++ adapter → physics engine → new simulation state → renderer → GPU to viewport.
- AI discovery events (anomaly detected) → push notification to QML via signal → displayed in a non‑intrusive popup or log panel.
- Parameter sweep status (long‑running task) → progresses via QtConcurrent → updates progress bar in sidebar.
...
- Set up Qt 6.5+ project with QML application template.
- Integrate existing `CMakeLists.txt` to build static libraries (`quantumverse_core`, `quantumverse_physics`, etc.) and link against Qt.
- Create a minimal QML window with an empty OpenGL viewport (using `QQuickFramebufferObject` or embedding `QOpenGLWidget` via `QWidget::createWindowContainer`).
...
|-------|------------|---------------|
| UI framework | Qt 6.5+ with Qt Quick (QML) | Modern, declarative UI; excellent OpenGL integration; cross‑platform |
| 3D rendering | OpenGL 4.5 via QQuickFramebufferObject | Leverages existing C++ renderers |
| Backend | Existing C++17 modules (physics, math, quantum gravity, AI) | No rewrite needed |
| Data binding | Qt properties / models | Seamless C++‑to‑QML communication |
| Build system | CMake 3.16+ (existing) with Qt CMake integration | Consistent with current project |
| Optional toolkits | Qt Charts (for discovery plots), Qt Data Visualization (for 3D plots in side panels) |  
...
|------|------------|--------|------------|
| Performance drop with 4D rotation + QML overhead | Medium | High | Profile early; use threaded physics simulation; keep QML scene light |
| Planck microscope LOD transitions cause stutter | Low | Medium | Pre‑load all LOD data; smooth blending shaders |
| Steep learning curve for QML integration with OpenGL | Medium | Medium | Start with a simple testbed; leverage community examples |
...
1. **Immediate**: Create a prototype QML application with an empty OpenGL viewport and a rotating Schwarzschild black hole.
2. **Week 1–2**: Draft detailed UI mockups (using Figma or similar) and validate with stakeholders.
3. **Then**: Begin Phase 0 and Phase 1 in parallel (foundation + basic navigation).
